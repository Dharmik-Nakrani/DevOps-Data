...
def run(self):
    ...
    while True:
        if self.stop:  # Don't trade if payout < 75%
            time.sleep(1)
            continue
        ...
        # Use last full candle
        last_candle = self.history[-1]
        direction = last_candle.get("stat")

        if direction in ("call", "put"):
            print(f"[ STRATEGY ] Executing trade: {direction.upper()} | Amount: {bet_amount}")
            self.getBid(direction, bet_amount)

            # Wait for trade to complete (~1.5 minute delay)
            time.sleep(95)

            # Check balance difference to see if trade was won/lost
            current_balance = self.getCurrentBalance()
            if hasattr(self, 'last_balance'):
                diff = current_balance - self.last_balance
                if diff < 0:
                    print(f"[ STRATEGY ] Trade lost. Increasing amount.")
                    bet_amount *= 2  # Martingale step
                else:
                    print(f"[ STRATEGY ] Trade won. Resetting amount.")
                    bet_amount = self.amount  # Reset to base
            self.last_balance = current_balance
        else:
            print("[ STRATEGY ] Skipped due to unknown candle state")
